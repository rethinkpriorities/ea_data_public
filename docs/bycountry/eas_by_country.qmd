...
log_features_percap <-  c("log(gdppop_3yr_avg)", "d_uk_english", "english_av",  "Emancipative", "Secular")
...
ea_dev_percap <- eas_by_country_all_m %>%  
  ggplot() +
  aes(x=pred_eapercap, y=easpercap_per100k, color=english_av) +
    devplotopts +
...
ea_dev_percap_i <- eas_by_country_all_m_i %>%  
  ggplot() +
  aes(x=pred_eapercap_i, y=easpercap_per100k, color=english_av) +
    devplotopts +
...
ea_dev_log <- model_log_linear_p1 %>% 
    augment(newdata=eas_by_country_all) %>% 
  ggplot() +
  aes(x=.fitted, y= log_eas_3yr_01, color=english_av) +
    devplotopts +
...
ea_dev_qp <- eas_by_country_all_m %>%  
  ggplot() +
  aes(x=pred_psn, y=eas_3yr_avg, color=english_av) +
    scale_x_continuous(trans='log2') +  
    scale_y_continuous(trans='log2') +
    devplotopts +
...
ea_outliers_qp_i <- eas_by_country_all_m_i %>%
  mutate(.resid =  eas_3yr_avg - pred_psn_i) %>% 
    ...
    filter(!(eas_3yr_avg<minEA_threshold & pred_psn < minEA_threshold)) %>% 
  ggplot() +
  aes(x=pred_psn_i, y=(eas_3yr_avg), color=english_av) +
    scale_x_continuous(trans='log2') +  
    scale_y_continuous(trans='log2') +
   devplotopts +
...
Unfortunately, it makes a new sheet each time; need to find a fix. 

```{r}
#! results: false
#! label: export

# Don't need authorisation to access this googlesheets file
